...
class TestOpenAPI(CommonCase):
    ...
    def test_usuarios_api(self):
        usuarios_service = self.public_services_env.component(usage="usuarios")
        openapi_def = usuarios_service.to_openapi()
        canonical_def = get_canonical_json("usuarios_api.json")
        self._fix_server_url(canonical_def)
        self.assertFalse(jsondiff.diff(openapi_def, canonical_def))