...
class ProductImageService(Component):
    ...
    _description = """
        Servicio de imagen para productos.
        Es utilizado para recuperar la imagen del producto
    ...
    def get(self, _id, size):
        ...
        status, headers, content = self.env["ir.http"].binary_content(
            model="product.template", id=_id, field=field, env=self.env
        )
        if not content:
            raise MissingError(_("No image found for product %s") % _id)
